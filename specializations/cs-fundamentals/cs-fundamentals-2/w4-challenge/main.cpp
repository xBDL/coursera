...
void downHeap(Node *n) {
  ...
  int leftVal = -1;
  int rightVal = -1;
  if (n->left  != nullptr)  leftVal = n->left->value;
  if (n->right != nullptr) rightVal = n->right->value;

  if (leftVal > 0 && (leftVal <= rightVal || rightVal < 0)) {  // swap left
    if (n->value > leftVal) {
  ...
  else if (rightVal > 0 && (rightVal <= leftVal || leftVal < 0)) {  // swap right
    if (n->value > rightVal) {
